    ...
    <div class="com-dial-box">
      ...
      <div class="content">
        <div class="content-header">
          <div>
            <div class="prize">{{state.mid_pointer.gift_name}}</div>
            <div class="tip">本站奖励</div>
          </div>
          <img class="arrow" src="/static/images/arrow.png" alt="" srcset="" mode="aspectFill">
          <div>
            <div class="prize">{{state.mid_pointer.next_pointer_gift_name}}</div>
        ...
        <div v-if="state.mid_pointer.is_has_prize">
          <div class="rank">作为本站前{{state.mid_pointer.rank}}名冲线者</div>
        ...
        <div v-else>
          <div class="rank">您是本站{{state.mid_pointer.rank}}名达到的用户</div>
          <div class="select-tip">该站点的奖励名额已经领取完毕</div>
        </div>
      </div>
      <div class="btns">
        <div class="btn"> {{state.mid_pointer.is_has_prize ? '考虑好了吗？请选择！':  '知道了，冲刺下一站'}}</div>
      </div>
